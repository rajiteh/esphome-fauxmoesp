...
#define DEBUG_MSG_FAUXMO(fmt, ...) \
    ESP_LOGD(DEBUG_FAUXMO_TAG, fmt, ##__VA_ARGS__)
...
class FauxmoDevice {
   public:
    ...
    void add_on_state_callback(
        std::function<void(uint8_t, const char*, bool, uint8_t)> callback) {
        callbacks_.push_back(callback);
    }

    void trigger_callbacks(uint8_t device_id, const char* device_name,
                           bool state, uint8_t value) {
   ...
   protected:
    std::string name_;
    uint8_t id_{0};
    std::vector<std::function<void(uint8_t, const char*, bool, uint8_t)>>
        callbacks_;
};
...
class FauxmoStateTrigger : public Trigger<uint8_t, std::string, bool, uint8_t> {
   public:
    explicit FauxmoStateTrigger(FauxmoDevice* parent) {
        parent->add_on_state_callback([this](uint8_t device_id,
...
class FauxmoESPComponent : public Component {
   public:
    ...
    void set_create_server(bool create_server) {
   ...
   protected:
    esp_netif_t* get_network_interface_();
    bool get_ip_(IPAddress& ip);
    bool get_mac_(char* mac_str, size_t len);
    ...
    bool is_initialized_{
        false};  // True once WiFi is ready and fauxmo is enabled
};
